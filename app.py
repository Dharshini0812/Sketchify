...
def feedback():
    ...
    user_language = request.form.get('language', 'en')  # Get user-selected language, default to English
    
    if feedback == "no":
        user_caption = request.form['user_caption']
        
        # Translate user feedback to English for consistency
        translated_feedback = translator.translate(user_caption, dest='en').text
        feedback_data[filename] = translated_feedback

        # Save feedback
        with open(FEEDBACK_FILE, 'w') as f:
            json.dump(feedback_data, f)

        return redirect(url_for('upload_file', message=translator.translate("Your suggestion is noted!", dest=user_language).text))
    
    return redirect(url_for('upload_file', message=translator.translate("Thank you for your suggestion!", dest=user_language).text))
